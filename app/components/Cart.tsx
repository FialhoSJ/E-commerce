...
export default function Cart() {
  ...
  return <>
    ...
    {open && mounted && createPortal(
      <div className="fixed inset-0 z-[60] bg-slate-950/60" onClick={() => setOpen(false)}>
        <aside className="absolute right-0 top-0 flex h-full w-full max-w-md flex-col overflow-hidden bg-white p-6 text-slate-900 shadow-2xl" onClick={(event) => event.stopPropagation()}>
                  ...
                  <div key={item.id} className="flex gap-3 border-b pb-4">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-2 flex items-center gap-2">
                        ...
                        <button onClick={() => updateQuantity(item.id, item.quantity + 1)} className="rounded bg-slate-100 px-2">+</button>
                        <button onClick={() => removeFromCart(item.id)} className="ml-auto text-xs text-red-600">Remover</button>
                      </div>
                    </div>
                  </div>
                ))}
